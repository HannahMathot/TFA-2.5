"use strict";
//sources :
//slider + hammer : HEAJ DWT B3G
//notions de callback et de return, relecture : entraide
//range normal : https://www.youtube.com/watch?v=YE10hytBBho
//range rond : https://codepen.io/om-10/pen/dyNBLOd
//tout le reste : moi

if (localStorage.getItem('end', true) == null) {
    localStorage.clear();
} else {
    document.querySelector(".warning").classList.add("hidden");
    document.querySelector(".main-image").classList.add("hidden");
    document.querySelector(".slider").classList.add("hidden");
    document.querySelector(".compteur").classList.add("hidden");
...
let warningEls = document.querySelectorAll(".warning__el");
...
warningButton.addEventListener("click", () => {
    ...
    warning.classList.add("back");
});

//Slider
let slider = document.querySelector(".slider");
let produitsSlider = document.querySelector(".slider__flex");
let previousButton = document.querySelector(".slider__button--previous");
let produitsButton = document.querySelector(".slider__button--next");
let produitsCompteur = 123;

previousButton.addEventListener("click", previous);
produitsButton.addEventListener("click", next);

const hammerSlider = new Hammer(produitsSlider);

hammerSlider.on("swipeleft", next);
hammerSlider.on("swiperight", previous);

function next(){
    if (produitsCompteur > -912) {
        produitsCompteur = produitsCompteur-115;
        produitsSlider.style.marginLeft = produitsCompteur+"px";
        console.log(produitsCompteur)
    }
}
function previous(){
    if (produitsCompteur <= 8) {
        produitsCompteur = produitsCompteur+115;
        produitsSlider.style.marginLeft = produitsCompteur+"px";
    }
}
...
let smallTitle = document.querySelector(".tiny-title");
...
let produits = [
    {name: "lessive", state: false, callback: () => toLessives()},
    {name: "adoucissant", state: false, callback: () => toLessives()},
    {name: "vitres", state: false, callback: () => toVitres()},
    {name: "wc", state: false, callback: () => toWC()},
    {name: "tablettes", state: false, callback: () => toTablettes()},
    {name: "multi", state: false, callback: () => toMulti()},
    {name: "degraissant", state: false, callback: () => toDegraissant()},
    {name: "desodorisant", state: false, callback: () => toDesodorisant()},
    {name: "detartrant", state: false, callback: () => toDetartrant()},
    {name: "liquide", state: false, callback: () => toLiquide()},
]

for (const produitButton of produitButtons) {
    produitButton.addEventListener("click", toogleProduit);
}
for (const armoireEl of armoireEls) {
    armoireEl.addEventListener("click", toogleProduit);
}

function toogleProduit(e) {
    let currentProduit = e.currentTarget;
    let currentClass = currentProduit.className.replace('slider__', '').replace('armoire__el', '').replace('armoire__', '');
    currentClass = currentClass.trim();
    let produit = produits.find(item => item.name == currentClass);

    if (produit.state){
        produit.state = false;
        goOnorNot();
        currentProduit.classList.add("hidden");
        document.querySelector(".slider__" + currentClass).classList.remove("no-background");
        document.querySelector(".slider__" + currentClass).classList.remove("no-background--" + currentClass);
        localStorage.removeItem(currentClass, true);
    } else {
        produit.state = true;
        goOnorNot();
        currentProduit.classList.add("no-background");
        currentProduit.classList.add("no-background--" + currentClass);
        document.querySelector(".armoire__" + currentClass).classList.remove("hidden");
        localStorage.setItem(currentClass, true);
    }
}

function goOnorNot(){
    if (produits.some(produit => produit.state == true)) {
...
function toEtapeDeux() {
    ...
    bigTitle.innerHTML = "Étape 2 sur 2";
    changeProduit();
}


let lessiveTrue = false;
function changeProduit() {
    let choice = produits.find(produit => produit.state == true);
    if (choice && choice.name == "adoucissant" && lessiveTrue) {
        choice.state = !choice.state;
        choice = produits.find(produit => produit.state == true);
    }
    if (choice) {
        if (choice.name == "lessive") {
            lessiveTrue = true;
        }
        choice.state = !choice.state;
...
function toLessives() {
    smallTitle.innerHTML = "Indiquez combien de lessives vous faites par mois.";
    ...
    mainImage.classList.add("main-image--lessive");
}
function toVitres() {
    smallTitle.innerHTML = "Faites glisser l'éponge pour indiquer combien de fois par an vous lavez vos fenêtres.";
    ...
    mainImage.classList.add("main-image--vitres");
}
function toWC() {
    smallTitle.innerHTML = "Faites tourner le produit pour montrer combien de fois par mois vous nettoyez vos wc.";
    ...
    mainImage.classList.add("main-image--wc");
}
function toTablettes() {
    smallTitle.innerHTML = "Indiquez combien de lave-vaisselles vous faites par semaine.";
    ...
    mainImage.classList.add("main-image--tablettes");
}
function toMulti() {
    smallTitle.innerHTML = "Faites glisser le produit pour indiquer combien de fois par mois vous utilisez votre produit multi-usage.";
...
function toDegraissant() {
    smallTitle.innerHTML = "Faites tourner le produit pour montrer combien de fois par mois vous utilisez votre dégraissant.";
...
function toDesodorisant() {
    smallTitle.innerHTML = "Indiquez combien de fois par mois vous désodorisez votre maison.";
...
function toDetartrant() {
    smallTitle.innerHTML = "Faites glisser l'éponge pour indiquer combien de fois par an vous utilisez votre détartrant.";
...
function toLiquide() {
    smallTitle.innerHTML = "Faites tourner l'éponge pour montrer combien de fois par mois vous utilisez du liquide vaisselle.";
...
function addNumber() {
    ...
    localStorage.setItem('compteur-final-number', currentNumberNumber);
    currentNumber.innerHTML = localStorage.getItem('compteur-final-number');
    if (smallTitle.textContent == "Indiquez combien de lessives vous faites par mois.") {
        localStorage.setItem('lessive-number', currentNumberNumber);
    } else if (smallTitle.textContent == "Indiquez combien de lave-vaisselles vous faites par semaine.") {
        localStorage.setItem('tablettes-number', currentNumberNumber);
    } else if (smallTitle.textContent == "Indiquez combien de fois par mois vous désodorisez votre maison.") {
        localStorage.setItem('desodorisant-number', currentNumberNumber);
    }
}
...
function slideRange() {
    ...
    localStorage.setItem('slider-range-normal-final-value', this.value);
    if (smallTitle.textContent == "Faites glisser l'éponge pour indiquer combien de fois par an vous lavez vos fenêtres.") {
        localStorage.setItem('vitres-number', this.value);
    } else if (smallTitle.textContent == "Faites glisser le produit pour indiquer combien de fois par mois vous utilisez votre produit multi-usage.") {
        localStorage.setItem('multi-number', this.value);
    } else if (smallTitle.textContent == "Faites glisser l'éponge pour indiquer combien de fois par an vous utilisez votre détartrant.") {
        localStorage.setItem('detartrant-number', this.value);
    }
...
$("#round").roundSlider({
    ...
    drag: function (e) {
        ...
        if (smallTitle.textContent == "Faites tourner le produit pour montrer combien de fois par mois vous nettoyez vos wc.") {
            localStorage.setItem('wc-number', e.value);
        } else if (smallTitle.textContent == "Faites tourner le produit pour montrer combien de fois par mois vous utilisez votre dégraissant.") {
            localStorage.setItem('degraissant-number', e.value);
        } else if (smallTitle.textContent == "Faites tourner l'éponge pour montrer combien de fois par mois vous utilisez du liquide vaisselle.") {
...
function toGain() {
    ...
    if ((localStorage.getItem('lessive', true)) && (localStorage.getItem('adoucissant') === null)) {
        ...
        lessiveIndustriel = (Math.ceil(lessiveIndustriel * 100) / 100);
        lessiveRecettes = (Math.ceil(lessiveRecettes * 100) / 100);
        ...
        localStorage.setItem('lessive-recettes', lessiveRecettes);
        localStorage.setItem('lessive-industriel', lessiveIndustriel);
        ...
        adoucissantIndustriel = (Math.ceil(adoucissantIndustriel * 100) / 100);
        adoucissantRecettes = (Math.ceil(adoucissantRecettes * 100) / 100);
        ...
        localStorage.setItem('adoucissant-recettes', adoucissantRecettes);
        localStorage.setItem('adoucissant-industriel', adoucissantIndustriel);
        ...
        lessiveRecettes = (Math.ceil(lessiveRecettes * 100) / 100);
        adoucissantRecettes = (Math.ceil(adoucissantRecettes * 100) / 100);
        let lessiveAdoucissantIndustriel = lessiveIndustriel + adoucissantIndustriel;
        let lessiveAdoucissantRecettes = lessiveRecettes + adoucissantRecettes;
        lessiveAdoucissantIndustriel = (Math.ceil(lessiveAdoucissantIndustriel * 100) / 100);
        lessiveAdoucissantRecettes = (Math.ceil(lessiveAdoucissantRecettes * 100) / 100);
    ...
    if (localStorage.getItem('vitres', true)) {
        ...
        vitresIndustriel = (Math.ceil(vitresIndustriel * 100) / 100);
        vitresRecettes = (Math.ceil(vitresRecettes * 100) / 100);
        ...
        localStorage.setItem('vitres-recettes', vitresRecettes);
        localStorage.setItem('vitres-industriel', vitresIndustriel);
    }
    if (localStorage.getItem('wc', true)) {
        ...
        wcIndustriel = (Math.ceil(wcIndustriel * 100) / 100);
        wcRecettes = (Math.ceil(wcRecettes * 100) / 100);
        ...
        localStorage.setItem('wc-recettes', wcRecettes);
        localStorage.setItem('wc-industriel', wcIndustriel);
    }
    if (localStorage.getItem('tablettes', true)) {
        ...
        tablettesIndustriel = (Math.ceil(tablettesIndustriel * 100) / 100);
        tablettesRecettes = (Math.ceil(tablettesRecettes * 100) / 100);
        ...
        localStorage.setItem('tablettes-recettes', tablettesRecettes);
        localStorage.setItem('tablettes-industriel', tablettesIndustriel);
    }
    if (localStorage.getItem('multi', true)) {
        ...
        multiIndustriel = (Math.ceil(multiIndustriel * 100) / 100);
        multiRecettes = (Math.ceil(multiRecettes * 100) / 100);
        ...
        localStorage.setItem('multi-recettes', multiRecettes);
        localStorage.setItem('multi-industriel', multiIndustriel);
    }
    if (localStorage.getItem('degraissant', true)) {
        ...
        degraissantIndustriel = (Math.ceil(degraissantIndustriel * 100) / 100);
        degraissantRecettes = (Math.ceil(degraissantRecettes * 100) / 100);
        ...
        localStorage.setItem('degraissant-recettes', degraissantRecettes);
        localStorage.setItem('degraissant-industriel', degraissantIndustriel);
    }
    if (localStorage.getItem('desodorisant', true)) {
        ...
        desodorisantIndustriel = (Math.ceil(desodorisantIndustriel * 100) / 100);
        desodorisantRecettes = (Math.ceil(desodorisantRecettes * 100) / 100);
        ...
        localStorage.setItem('desodorisant-recettes', desodorisantRecettes);
    ...
    if (localStorage.getItem('detartrant', true)) {
        ...
        detartrantIndustriel = (Math.ceil(detartrantIndustriel * 100) / 100);
        detartrantRecettes = (Math.ceil(detartrantRecettes * 100) / 100);
        ...
        localStorage.setItem('detartrant-recettes', detartrantRecettes);
        localStorage.setItem('detartrant-industriel', detartrantIndustriel);
    }
    if (localStorage.getItem('liquide', true)) {
        let liquideIndustriel = (parseInt(localStorage.getItem('liquide-number'), 10) * 0.1 * 12 * 1.69);
        let liquideRecettes = (parseInt(localStorage.getItem('liquide-number'))*0);
        liquideIndustriel = (Math.ceil(liquideIndustriel * 100) / 100);
        liquideRecettes = (Math.ceil(liquideRecettes * 100) / 100);
        ...
        localStorage.setItem('liquide-recettes', liquideRecettes);
        localStorage.setItem('liquide-industriel', liquideIndustriel);
    }
    localStorage.setItem('final-number', finalNumber);
    localStorage.setItem('industriel-number', industrielNumber);
    localStorage.setItem('recettes-number', recettesNumber);
    results()
}

function results(){
    location.replace("http://hannah-mathot.be/projets/tfa/results.html");
}